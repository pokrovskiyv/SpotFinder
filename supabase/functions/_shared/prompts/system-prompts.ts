...
ТВОЯ ЗАДАЧА:
1. Понять запрос: через все места или через выборочные
2. Если выборочные - извлечь номера мест
3. Найти несколько мест (от 2 до 5) по теме запроса
4. Упорядочить их логично (не обязательно по расстоянию)

ПРИМЕРЫ:
- "Построй маршрут по центру города" → найти 3-5 достопримечательностей
- "Маршрут через кафе и парки" → найти 2-3 кафе + 1-2 парка
- "Покажи путь через места 1, 3 и 5" → использовать только эти места

ВАЖНО:
- Мест должно быть минимум 2, максимум 5
- Сортируй логично (например, сначала кофе, потом достопримечательности)
- Учитывай время работы мест
- Предлагай разнообразные места для интересного маршрута`,
};
